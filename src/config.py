...
class DatabaseConfig(BaseSettings):
    ...
    host: str = Field(
        default="localhost", description="PostgreSQL server hostname", env="POSTGRES_HOST"
    )

    # Database server port
    port: int = Field(
        default=5432, ge=1, le=65535, description="PostgreSQL server port", env="POSTGRES_PORT"
    )

    # Database name
    database: str = Field(
        default="quantumedge", description="Database name for the application", env="POSTGRES_DB"
    )

    # Authentication username
    user: str = Field(
        default="qe_user", description="Database authentication username", env="POSTGRES_USER"
    )

    # Authentication password (should be changed in production!)
    password: str = Field(
        default="qe_pass", description="Database authentication password", env="POSTGRES_PASSWORD"
    )

    # Connection pool size for async operations
    pool_size: int = Field(
        default=10, ge=1, description="Number of connections to maintain in pool"
    )

    # Maximum overflow connections for burst traffic
    max_overflow: int = Field(
        default=20, ge=0, description="Additional connections beyond pool_size"
    )

    # Connection timeout in seconds
    pool_timeout: float = Field(
        default=30.0, ge=1.0, description="Seconds to wait for available connection"
    )
...
class EdgeProfile(BaseSettings):
    ...
    power_budget_watts: float = Field(ge=1.0, description="Power budget in watts")

    thermal_limit_celsius: float = Field(
        ge=40.0, le=120.0, description="Maximum temperature in Celsius"
    )

    memory_mb: int = Field(ge=512, description="Available memory in megabytes")

    cpu_cores: int = Field(ge=1, description="Number of CPU cores")

    max_execution_time_sec: int = Field(ge=1, description="Maximum execution time in seconds")

    network_latency_ms: int = Field(ge=1, description="Network latency in milliseconds")
...
class EdgeConfig(BaseSettings):
    ...
    memory_mb: Optional[int] = Field(
        default=None, ge=512, description="Override memory in megabytes", env="EDGE_MEMORY_MB"
    )

    # Custom overrides for CPU cores
    cpu_cores: Optional[int] = Field(
        default=None, ge=1, description="Override CPU cores", env="EDGE_CPU_CORES"
    )
    ...
    def get_profile(self, profile_name: str) -> EdgeProfile:
        ...
        if profile_name not in self.profiles:
            raise KeyError(
                f"Invalid profile '{profile_name}'. Must be one of {list(self.profiles.keys())}"
            )
...
class APIConfig(BaseSettings):
    ...
    host: str = Field(default="0.0.0.0", description="API server bind address", env="API_HOST")

    # API server port
    port: int = Field(
        default=8000, ge=1024, le=65535, description="API server port", env="API_PORT"
    )

    # Number of uvicorn worker processes
    workers: int = Field(
        default=4, ge=1, description="Number of uvicorn worker processes", env="API_WORKERS"
    )

    # Application name
    title: str = Field(
        default="QuantumEdge Pipeline API", description="API title displayed in docs"
    )
    ...
    debug: bool = Field(
        default=True, description="Enable debug mode with detailed errors", env="DEBUG"
    )
    ...
    docs_url: str = Field(default="/docs", description="Swagger UI documentation endpoint")

    # ReDoc documentation URL
    redoc_url: str = Field(default="/redoc", description="ReDoc documentation endpoint")
    ...
    cors_allow_credentials: bool = Field(default=True, description="Allow cookies in CORS requests")
    ...
    rate_limit_per_minute: int = Field(
        default=60, ge=1, description="Max requests per minute per IP address"
    )
...
class Settings(BaseSettings):
    ...
    environment: Literal["development", "staging", "production"] = Field(
        default="development", description="Deployment environment", env="APP_ENV"
    )
